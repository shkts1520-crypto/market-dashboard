(function () {
  ...
  function renderCore(view) {
    ...
    const cols=[{label:'#',key:'_rank'},{label:'銘柄',key:'ticker',ticker:true,align:'left'},{label:'Final',format:(r)=>num(r.final_score,1)},{label:'RS63',format:(r)=>num(r.rs63,1)},{label:'RS126',format:(r)=>num(r.rs126,1)},{label:'RS189',format:(r)=>num(r.rs189,1)},{label:'Price',format:(r)=>finite(r.price)===null?'—':`$${num(r.price,2)}`},{label:'DDV20',format:(r)=>finite(r.ddv20)===null?'—':`$${(r.ddv20/1e6).toFixed(0)}M`},{label:'Trend',format:(r)=>finite(r.sma50)!==null&&finite(r.sma200)!==null?(r.sma50>r.sma200?'50>200':'50≤200'):'—'}];
  ...
  function ensureBreadthQualityCard(view){const section=document.getElementById('t-rotation');if(!section)return;let node=section.querySelector('.card[data-v38-generated="breadth-quality"]');if(!node){const heads=Array.from(section.querySelectorAll('.msec'));const h=heads.find((x)=>String(x.textContent||'').includes('②'));node=el('div','card v38-breadth-quality');node.dataset.v38Generated='breadth-quality';if(h&&h.nextSibling)h.parentNode.insertBefore(node,h.nextSibling);else section.appendChild(node);}heading(node,'指数と中身の乖離','指数（時価総額加重）と中身（等加重・Breadth）の差を見る。上位数銘柄だけで指数が持ち上がっていないかを確認。');const pairs=el('div','v38-pair-grid');[['NASDAQ100','QQQ','QQQE'],['S&P500','SPY','RSP']].forEach(([name,cap,equal])=>{const a=marketSummary(view,cap),b=marketSummary(view,equal),gap1=finite(a.change_1w)!==null&&finite(b.change_1w)!==null?a.change_1w-b.change_1w:null,gapm=finite(a.change_1m)!==null&&finite(b.change_1m)!==null?a.change_1m-b.change_1m:null;const box=el('div','v38-pair');box.innerHTML=`<div class="v38-pair-title">${html(name)} <small>${cap} vs ${equal}</small></div><div class="v38-pair-values"><span>1週 ${pct(a.change_1w)} / ${pct(b.change_1w)}</span><b>乖離 ${pct(gap1)}</b></div><div class="v38-pair-values"><span>1カ月 ${pct(a.change_1m)} / ${pct(b.change_1m)}</span><b>乖離 ${pct(gapm)}</b></div>`;pairs.appendChild(box);});node.appendChild(pairs);const diag=((view.rotation||{}).diagnostics||{}).sector||[],bgrid=el('div','v38-breadth-grid');diag.slice().sort((a,b)=>(finite(b.above_sma50_pct)||-1)-(finite(a.above_sma50_pct)||-1)).slice(0,8).forEach((r)=>{const item=el('div','v38-breadth-row');const w=Math.max(0,Math.min(100,finite(r.above_sma50_pct)||0));item.innerHTML=`<span>${html(r.group)}</span><div class="v38-breadth-bar"><i style="width:${w}%"></i></div><b>${num(w,0)}%</b>`;bgrid.appendChild(item);});node.appendChild(bgrid);mark(node,'data/ui_view_model.json.rotation.diagnostics.sector+daily.market_summaries','READY');}
  function renderRotation(view) {
    ...
    richTable(card('t-rotation','主導セクター・業種'),'主導セクター・業種',[{label:'#',key:'_rank'},{label:'セクター',key:'group',align:'left'},{label:'RS63',format:(r)=>num(r.rs63_avg,1)},{label:'1カ月',format:(r)=>pct(r.ret20_avg)},{label:'50日線上',format:(r)=>finite(r.above_sma50_pct)===null?'—':`${num(r.above_sma50_pct,0)}%`},{label:'銘柄数',key:'member_count'}],sectors.slice(0,18).map((r,i)=>({...r,_rank:i+1})),'data/ui_view_model.json.rotation.diagnostics.sector','ETFではなく構成銘柄の実データから算出。強さと広がりを同時に見る。');
    const leaderRows=[];industries.slice(0,12).forEach((r)=>(r.leaders||[]).slice(0,3).forEach((ticker)=>leaderRows.push({ticker,group:r.group,theme_rs:r.theme_rs,rs63_avg:r.rs63_avg,ret20_avg:r.ret20_avg})));richTable(card('t-rotation','強い業種の主導株'),'強い業種の主導株',[{label:'Ticker',key:'ticker',ticker:true,align:'left'},{label:'業種',key:'group',align:'left'},{label:'テーマRS',format:(r)=>num(r.theme_rs,1)},{label:'業種RS63',format:(r)=>num(r.rs63_avg,1)},{label:'1M',format:(r)=>pct(r.ret20_avg)}],leaderRows,'data/ui_view_model.json.rotation.diagnostics.industry.leaders');
    richTable(card('t-rotation','セクターETF強弱'),'セクターETF強弱',[{label:'順位',key:'rank'},{label:'ETF',key:'ticker',ticker:true,align:'left'},{label:'日',format:(r)=>pct(r.d1)},{label:'週',format:(r)=>pct(r.w1)},{label:'月',format:(r)=>pct(r.m1)}],SECTORS.map((ticker)=>({ticker,d1:marketSummary(view,ticker).change_1d,w1:marketSummary(view,ticker).change_1w,m1:marketSummary(view,ticker).change_1m})).sort((a,b)=>(finite(b.m1)||-99)-(finite(a.m1)||-99)).map((r,i)=>({...r,rank:i+1})),'data/ui_view_model.json.daily.market_summaries');
    richTable(card('t-rotation','サブテーマ別RS'),'サブテーマ別RS（ユニバース内）',[{label:'#',key:'rank'},{label:'サブテーマ',key:'theme_name',align:'left'},{label:'Theme RS',format:(r)=>num(r.theme_rs,1)},{label:'RS63',format:(r)=>num(r.rs63_median,1)},{label:'RS189',format:(r)=>num(r.rs189_median,1)},{label:'1カ月',format:(r)=>pct(r.ret20_median)},{label:'主導株',format:(r)=>(r.leaders||[]).slice(0,3).join(' · ')}],fine.slice(0,24).map((r,i)=>({...r,rank:i+1})),'data/ui_view_model.json.rotation.fine_theme_rows');
    const thermometer=card('t-rotation','テーマETFの温度計');if(thermometer){heading(thermometer,'テーマ温度計','現行Theme診断を日本語主体で表示。元ネタのテーマETF RRGとは別物なので混同しない。');const bars=el('div','v38-theme-bars');major.slice(0,16).forEach((r,i)=>{const score=finite(r.theme_rs);const row=el('div','v38-theme-bar');row.innerHTML=`<span class="rk">${i+1}</span><span class="name">${html(r.group)}</span><div class="track"><i style="width:${Math.max(0,Math.min(100,score||0))}%"></i></div><b>${num(score,1)}</b><small>${(r.leaders||[]).slice(0,3).map(html).join(' · ')}</small>`;bars.appendChild(row);});thermometer.appendChild(bars);mark(thermometer,'data/ui_view_model.json.rotation.major_theme_groups','READY');}
  }
