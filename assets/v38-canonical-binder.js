(function () {
  ...
  function upwardRows(options,bucket) {
    const raw=(((options||{}).upward_rankings)||{})[bucket];
  ...
  function renderUpwardOptions(section,options) {
    if(!section)return;
    let cardNode=section.querySelector('.card[data-v38-generated="options-upward"]');
    if(!cardNode){
      cardNode=el('div','card v38-options-upward');
      cardNode.dataset.v38Generated='options-upward';
      const first=section.querySelector('.card');
      if(first&&first.parentNode)first.parentNode.insertBefore(cardNode,first);
      else section.appendChild(cardNode);
    }
    heading(cardNode,'上方向配置 Upward Positioning','既存の実オプション4条件で抽出した表示専用ランキング。上昇予測・売買ゲートではありません。0–45DTEを総合として先頭表示。');
    const groups=el('div','v38-options-upward-groups');
    ['0-45','0-6','7-21','22-45'].forEach((bucket)=>{
      const rows=upwardRows(options,bucket),group=el('div','v38-options-upward-group'),head=el('div','v38-options-upward-head');
      head.append(el('b','',bucket.replace('-','–')+' DTE'),el('span','v38-options-upward-count',rows.length+'銘柄'));
      group.appendChild(head);
      if(!rows.length){
        group.appendChild(el('div','empty','該当なし'));
      } else {
        rows.slice(0,12).forEach((r,index)=>{
          const line=el('div','v38-options-upward-row');
          line.append(
            el('span','rsx-rk',finite(r.upward_rank)!==null?r.upward_rank:index+1),
            tickerButton(r.ticker),
            el('span','v38-options-upward-score',String(r.upward_structure_score||0)+'/'+String(r.upward_structure_observed||0)),
            el('span','v38-options-upward-geom',`C/P ${optionRatio(r.call_put_gex_ratio)} · Spot ${num(r.spot,2)} / Flip ${num(r.gamma_flip,2)} / Call ${num(r.call_wall,2)}`)
          );
          group.appendChild(line);
        });
        if(rows.length>12)group.appendChild(el('div','empty',`上位12件を表示（全${rows.length}件）`));
      }
      groups.appendChild(group);
    });
    cardNode.appendChild(groups);
    mark(cardNode,'data/options/index.json.upward_rankings','READY');
  }
  ...
  function renderOptions(options) {
    ...
    renderUpwardOptions(section,options);
