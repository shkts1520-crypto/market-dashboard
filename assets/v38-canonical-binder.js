(function () {
  ...
  function renderCore(view) {
    ...
    const ranked=rows.map((r,i)=>({...r,_rank:finite(r.rank)!==null?r.rank:i+1}));
  ...
  function renderMoneyFlow(cardNode, view) {
    ...
    heading(cardNode,'資金フロー（GICS11＋スタイル）','横=SPY比63営業日相対力、縦=10営業日RSモメンタム。中央100。元ネタ同様に4象限で読む。');
    ...
    [['主導 Leading',W-24,28,'end'],['改善 Improving',24,28,'start'],['停滞 Lagging',24,H-18,'start'],['弱化 Weakening',W-24,H-18,'end']].forEach(([txt,x,y,anc])=>{const t=document.createElementNS(svg.namespaceURI,'text');t.textContent=txt;t.setAttribute('x',x);t.setAttribute('y',y);t.setAttribute('text-anchor',anc);t.setAttribute('class','v38-rrg-quadrant');svg.appendChild(t);});
    ...
    rows.forEach((r)=>{const x=finite(r.x),y=finite(r.y);if(x===null||y===null)return;const g=document.createElementNS(svg.namespaceURI,'g');g.dataset.v38Ticker=r.ticker||'';const c=document.createElementNS(svg.namespaceURI,'circle');c.setAttribute('cx',SX(x));c.setAttribute('cy',SY(y));c.setAttribute('r',SECTORS.includes(r.ticker)?10:7);c.setAttribute('class',y>=100?'v38-rrg-point up':'v38-rrg-point down');const t=document.createElementNS(svg.namespaceURI,'text');t.textContent=r.ticker;t.setAttribute('x',SX(x)+12);t.setAttribute('y',SY(y)+4);t.setAttribute('class','v38-rrg-label');g.append(c,t);svg.appendChild(g);});
    ...
    const plot=el('div','v38-rrg-source');plot.appendChild(svg);cardNode.appendChild(plot);const legend=el('div','v38-rrg-legend');legend.innerHTML='<span><i class="gics"></i>GICS11</span><span><i class="style"></i>スタイルETF</span><span>中心=100</span>';cardNode.appendChild(legend);mark(cardNode,'data/ui_view_model.json.rotation.money_flow','READY');
  }
  function renderRotationHeatmap(cardNode,view){if(!cardNode)return;heading(cardNode,'セクター温度マップ Sector Heatmap','前日・1週・1カ月を切替。色だけでなく値も表示。');const controls=el('div','v38-heat-controls'),grid=el('div','v38-sector-heatmap v38-rotation-heat');const periods=[['1D','change_1d'],['1W','change_1w'],['1M','change_1m']];function draw(key){grid.replaceChildren();SECTORS.forEach((ticker)=>{const s=marketSummary(view,ticker),v=finite(s[key]),cell=el('div','v38-sector-cell');cell.dataset.direction=v===null?'flat':v>=0?'up':'down';cell.style.setProperty('--heat',String(Math.min(1,Math.abs(v||0)/.06)));cell.append(el('b','',ticker),el('span','',pct(v)));grid.appendChild(cell);});}periods.forEach(([lab,key],i)=>{const b=el('button',i===1?'active':'',lab);b.type='button';b.dataset.v38HeatPeriod=key;b.addEventListener('click',()=>{controls.querySelectorAll('button').forEach((x)=>x.classList.toggle('active',x===b));draw(key);});controls.appendChild(b);});cardNode.append(controls,grid);draw('change_1w');mark(cardNode,'data/ui_view_model.json.daily.market_summaries','READY');}
  function ensureBreadthQualityCard(view){const section=document.getElementById('t-rotation');if(!section)return;let node=section.querySelector('.card[data-v38-generated="breadth-quality"]');if(!node){const heads=Array.from(section.querySelectorAll('.msec'));const h=heads.find((x)=>String(x.textContent||'').includes('②'));node=el('div','card v38-breadth-quality');node.dataset.v38Generated='breadth-quality';if(h&&h.nextSibling)h.parentNode.insertBefore(node,h.nextSibling);else section.appendChild(node);}heading(node,'広がり診断 Breadth Quality','指数の強さと、実際に50日線を上回る銘柄の広がりを同時確認。');const pairs=el('div','v38-pair-grid');[['NASDAQ100','QQQ','QQQE'],['S&P500','SPY','RSP']].forEach(([name,cap,equal])=>{const a=marketSummary(view,cap),b=marketSummary(view,equal),gap1=finite(a.change_1w)!==null&&finite(b.change_1w)!==null?a.change_1w-b.change_1w:null,gapm=finite(a.change_1m)!==null&&finite(b.change_1m)!==null?a.change_1m-b.change_1m:null;const box=el('div','v38-pair');box.innerHTML=`<div class="v38-pair-title">${html(name)} <small>${cap} vs ${equal}</small></div><div class="v38-pair-values"><span>1W ${pct(a.change_1w)} / ${pct(b.change_1w)}</span><b>Gap ${pct(gap1)}</b></div><div class="v38-pair-values"><span>1M ${pct(a.change_1m)} / ${pct(b.change_1m)}</span><b>Gap ${pct(gapm)}</b></div>`;pairs.appendChild(box);});node.appendChild(pairs);const diag=((view.rotation||{}).diagnostics||{}).sector||[],bgrid=el('div','v38-breadth-grid');diag.slice().sort((a,b)=>(finite(b.above_sma50_pct)||-1)-(finite(a.above_sma50_pct)||-1)).slice(0,8).forEach((r)=>{const item=el('div','v38-breadth-row');const w=Math.max(0,Math.min(100,finite(r.above_sma50_pct)||0));item.innerHTML=`<span>${html(r.group)}</span><div class="v38-breadth-bar"><i style="width:${w}%"></i></div><b>${num(w,0)}%</b>`;bgrid.appendChild(item);});node.appendChild(bgrid);mark(node,'data/ui_view_model.json.rotation.diagnostics.sector+daily.market_summaries','READY');}
  function renderRotation(view) {
    ...
    richTable(card('t-rotation','主導セクター・業種'),'主導セクター・業種 Leading Groups',[{label:'#',key:'_rank'},{label:'Group',key:'group',align:'left'},{label:'RS63',format:(r)=>num(r.rs63_avg,1)},{label:'1M',format:(r)=>pct(r.ret20_avg)},{label:'Breadth50',format:(r)=>finite(r.above_sma50_pct)===null?'—':`${num(r.above_sma50_pct,0)}%`},{label:'Members',key:'member_count'}],sectors.slice(0,18).map((r,i)=>({...r,_rank:i+1})),'data/ui_view_model.json.rotation.diagnostics.sector','Universe内部の強さと広がり。');
    const leaderRows=[];industries.slice(0,12).forEach((r)=>(r.leaders||[]).slice(0,3).forEach((ticker)=>leaderRows.push({ticker,group:r.group,theme_rs:r.theme_rs,rs63_avg:r.rs63_avg,ret20_avg:r.ret20_avg})));richTable(card('t-rotation','強い業種の主導株'),'強い業種の主導株 Leaders in Strong Groups',[{label:'Ticker',key:'ticker',ticker:true,align:'left'},{label:'Theme',key:'group',align:'left'},{label:'Theme RS',format:(r)=>num(r.theme_rs,1)},{label:'Group RS63',format:(r)=>num(r.rs63_avg,1)},{label:'1M',format:(r)=>pct(r.ret20_avg)}],leaderRows,'data/ui_view_model.json.rotation.diagnostics.industry.leaders');
    richTable(card('t-rotation','セクターETF強弱'),'セクターETF強弱 Sector ETF Strength',[{label:'#',key:'rank'},{label:'ETF',key:'ticker',ticker:true,align:'left'},{label:'1D',format:(r)=>pct(r.d1)},{label:'1W',format:(r)=>pct(r.w1)},{label:'1M',format:(r)=>pct(r.m1)}],SECTORS.map((ticker)=>({ticker,d1:marketSummary(view,ticker).change_1d,w1:marketSummary(view,ticker).change_1w,m1:marketSummary(view,ticker).change_1m})).sort((a,b)=>(finite(b.m1)||-99)-(finite(a.m1)||-99)).map((r,i)=>({...r,rank:i+1})),'data/ui_view_model.json.daily.market_summaries');
    richTable(card('t-rotation','サブテーマ別RS'),'サブテーマ別RS（ユニバース内）',[{label:'#',key:'rank'},{label:'Theme',key:'theme_name',align:'left'},{label:'Theme RS',format:(r)=>num(r.theme_rs,1)},{label:'RS63',format:(r)=>num(r.rs63_median,1)},{label:'RS189',format:(r)=>num(r.rs189_median,1)},{label:'1M',format:(r)=>pct(r.ret20_median)},{label:'Leaders',format:(r)=>(r.leaders||[]).slice(0,3).join(' · ')}],fine.slice(0,24).map((r,i)=>({...r,rank:i+1})),'data/ui_view_model.json.rotation.fine_theme_rows');
    const thermometer=card('t-rotation','テーマETFの温度計');if(thermometer){heading(thermometer,'テーマの温度計 Major Theme','Sector ETFの重複表示ではなく、現行Theme診断を表示。');const bars=el('div','v38-theme-bars');major.slice(0,16).forEach((r,i)=>{const score=finite(r.theme_rs);const row=el('div','v38-theme-bar');row.innerHTML=`<span class="rk">${i+1}</span><span class="name">${html(r.group)}</span><div class="track"><i style="width:${Math.max(0,Math.min(100,score||0))}%"></i></div><b>${num(score,1)}</b><small>${(r.leaders||[]).slice(0,3).map(html).join(' · ')}</small>`;bars.appendChild(row);});thermometer.appendChild(bars);mark(thermometer,'data/ui_view_model.json.rotation.major_theme_groups','READY');}
  }
  ...
  async function renderRs(view) {
    ...
    [63,126,189].forEach((period)=>{const rows=windows[String(period)]||[];richTable(card('t-rs',`RS${period} Top10`),`RS${period} Top10`,[{label:'#',key:'rank'},{label:'Ticker',key:'ticker',ticker:true,align:'left'},{label:'Industry',key:'industry',align:'left'},{label:'RS63',format:(r)=>num(r.rs63,1)},{label:'RS126',format:(r)=>num(r.rs126,1)},{label:'RS189',format:(r)=>num(r.rs189,1)},{label:'1M',format:(r)=>pct(r.ret20)},{label:'DDV',format:(r)=>r.ddv20_display||'—'}],rows,`data/ui_view_model.json.rs.windows.${period}`);});
    ...
    const cross=(rs.rows||[]).filter((r)=>finite(r.rs63)>=85&&finite(r.rs126)>=85&&finite(r.rs189)>=85);richTable(card('t-rs','三窓一致リーダー'),'三窓一致リーダー Cross-Window Leaders',[{label:'Ticker',key:'ticker',ticker:true,align:'left'},{label:'Industry',key:'industry',align:'left'},{label:'RS63',format:(r)=>num(r.rs63,1)},{label:'RS126',format:(r)=>num(r.rs126,1)},{label:'RS189',format:(r)=>num(r.rs189,1)},{label:'52W距離',format:(r)=>pct(r.dist52)}],cross,'data/ui_view_model.json.rs.rows');
  ...
  function renderOptions(options) {
    ...
    OPTION_BUCKETS.forEach((bucket)=>{
      const c=Array.from(section.querySelectorAll('.card')).find((node)=>originalTitle(node).includes(bucket));
      const rows=(buckets[bucket]&&buckets[bucket].rows)||[];
      simpleList(c,`DTE ${bucket}`,rows.slice(0,20).map((r)=>({ticker:r.ticker,meta:r.expiry||'',value:`Spot ${num(r.spot,2)} · Call ${num(r.call_wall,2)} · Flip ${num(r.gamma_flip,2)} · Put ${num(r.put_wall,2)} · EM ${finite(r.expected_move_pct)===null?'—':`±${(100*r.expected_move_pct).toFixed(1)}%`}`})),`data/options/index.json.buckets.${bucket}`,'実測Wall / Gamma Flip / Expected Moveのみを表示。');
