...
const LAYOUTS={
 ...
 't-rotation':[
  {label:'① ローテーションの向き（RRG）',en:'Rotation Direction (RRG)',q:'改善→主導→弱化→停滞の時計回り（視覚的な全体像）',cards:['セクター・ローテーション（テーマETF）']},
...
function applyStructure(){NON_OPTIONS.filter(id=>id!=='t-post1').forEach(sourceOrder);sourcePublish();sourceRsControl();document.body.dataset.v38UiAuthority='build_dashboard_3_py';}
