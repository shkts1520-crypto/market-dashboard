(function () {
  ...
  const STYLE_ID = 'v38-restored-chart-style';
  ...
  function svgFallback(host, candles, optionLevels, v63, v252, life) {
    host.replaceChildren();
    if (!candles.length) return;
    const width=900,height=560,px=48,py=30;
    const values = candles.flatMap((c) => [c.high,c.low]).concat(optionLevels.map((l)=>l.value), v63.map((x)=>x.value), v252.map((x)=>x.value), life === null ? [] : [life]);
    ...
    poly(v63,'#7a4fb5');poly(v252,'#397c78');
  ...
  function legend(modal, optionLevels, v63, v252, life) {
  ...
  async function render(modal, data, ticker, found) {
    ...
    const candles=candlesFor(data,ticker), optionLevels=levels(found && found.row), v63=rollingVwap(candles,63), v252=rollingVwap(candles,252), life=lifeValue(data,ticker);
    legend(modal,optionLevels,v63,v252,life);
    const status=modal.querySelector('.v38-rc-status');
    const bits=[];if(found&&found.bucket)bits.push(found.bucket+' DTE');if(found&&found.row&&finite(found.row.expected_move_pct)!==null)bits.push('Expected Move '+(found.row.expected_move_pct*100).toFixed(1)+'%');bits.push('63/252/All-time VWAPは観察線・売買ゲートではありません');status.textContent=bits.join(' • ');
    ...
    try{
      const lib=await loadLib();if(modal.hidden||activeTicker!==ticker)return;if(!lib||typeof lib.createChart!=='function'){svgFallback(host,candles,optionLevels,v63,v252,life);return;}
      ...
      addLine(v63,'#7a4fb5','VWAP63');addLine(v252,'#397c78','VWAP252');
    ...
    }catch(_){dispose();if(!modal.hidden&&activeTicker===ticker)svgFallback(host,candles,optionLevels,v63,v252,life);}
  }
  ...
  function openTicker(value) {
    ...
    const modal=ensureModal();activeTicker=ticker;modal.hidden=false;modal.querySelector('.v38-rc-title').textContent=ticker;modal.querySelector('.v38-rc-status').textContent='ローソク足・VWAP・Options水準を読み込み中…';modal.querySelector('.v38-rc-chart').replaceChildren();modal.querySelector('.v38-rc-legend').replaceChildren();
  ...
  function tickerFromTarget(target) {
    ...
    const el=target.closest('[data-v38-ticker],[data-v38-rs-ticker],[data-ticker],[data-symbol]');
    if(el){return cleanTicker(el.getAttribute('data-v38-ticker')||el.getAttribute('data-v38-rs-ticker')||el.getAttribute('data-ticker')||el.getAttribute('data-symbol'));}
  ...
  function isTickerTarget(target){return target instanceof Element&&Boolean(target.closest('[data-v38-ticker],[data-v38-rs-ticker],[data-ticker],[data-symbol],a.v38-ticker-link,a[href*="tradingview.com/chart"],.mvr-t,.tk,.chip'));}
