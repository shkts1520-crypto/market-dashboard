(function () {
  ...
  function fallbackRow(options, ticker) {
    ...
    if (row) return {bucket: String(row.bucket || '0-45'), row, historical: false};
  ...
  function resolveRow(options, ticker, bucket) {
    ...
    if (buckets.length) return {bucket: buckets[0], row: rowForBucket(options, ticker, buckets[0]), historical: false};
  ...
  function candlesFor(options, ticker) {
    const chart = options && options.chart_ohlc && typeof options.chart_ohlc === 'object' ? options.chart_ohlc[ticker] : null;
    return (Array.isArray(chart) ? chart : []).map((row) => {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      .v38-oc-fallback{width:100%;height:100%;display:block}.v38-oc-line{display:none}.v38-oc-price{display:none}
  ...
  function fallbackCandles(host, candles, levels) {
    ...
    const width = 760, height = 520, px = 44, py = 26;
    const lows = candles.map((c) => c.low).concat(levels.map((l) => l.value));
    const highs = candles.map((c) => c.high).concat(levels.map((l) => l.value));
    let low = Math.min.apply(null, lows), high = Math.max.apply(null, highs);
    const margin = Math.max((high - low) * .06, high * .005); low -= margin; high += margin;
    const x = (i) => px + (width - 2 * px) * (i + .5) / candles.length;
    const y = (v) => py + (high - v) / Math.max(1e-9, high - low) * (height - 2 * py);
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg'); svg.setAttribute('class', 'v38-oc-fallback'); svg.setAttribute('viewBox', `0 0 ${width} ${height}`); svg.setAttribute('preserveAspectRatio', 'none');
    const step = (width - 2 * px) / candles.length, bodyW = Math.max(1.2, Math.min(8, step * .58));
    candles.forEach((c, i) => { const xx=x(i), up=c.close>=c.open, col=up?'#2c9a7b':'#d9534f'; const wick=document.createElementNS(svg.namespaceURI,'line');wick.setAttribute('x1',xx.toFixed(1));wick.setAttribute('x2',xx.toFixed(1));wick.setAttribute('y1',y(c.high).toFixed(1));wick.setAttribute('y2',y(c.low).toFixed(1));wick.setAttribute('stroke',col);wick.setAttribute('stroke-width','1');svg.appendChild(wick);const rect=document.createElementNS(svg.namespaceURI,'rect');rect.setAttribute('x',(xx-bodyW/2).toFixed(1));rect.setAttribute('y',Math.min(y(c.open),y(c.close)).toFixed(1));rect.setAttribute('width',bodyW.toFixed(1));rect.setAttribute('height',Math.max(1,Math.abs(y(c.open)-y(c.close))).toFixed(1));rect.setAttribute('fill',up?col:'#fff');rect.setAttribute('stroke',col);rect.setAttribute('stroke-width','1');svg.appendChild(rect); });
    levels.forEach((level) => { const yy=y(level.value), line=document.createElementNS(svg.namespaceURI,'line'); line.setAttribute('x1',String(px));line.setAttribute('x2',String(width-px));line.setAttribute('y1',yy.toFixed(1));line.setAttribute('y2',yy.toFixed(1));line.setAttribute('stroke',level.color);line.setAttribute('stroke-width','1.3');line.setAttribute('stroke-dasharray',level.style==='solid'?'':'5 4');svg.appendChild(line);const text=document.createElementNS(svg.namespaceURI,'text');text.setAttribute('x',String(width-px));text.setAttribute('y',(yy-3).toFixed(1));text.setAttribute('fill',level.color);text.setAttribute('font-size','11');text.setAttribute('font-weight','800');text.setAttribute('text-anchor','end');text.textContent=level.label+' '+money(level.value);svg.appendChild(text); });
  ...
  async function renderCandles(modal, options, ticker, row) {
    ...
    if (!candles.length) {
      const unavailable = document.createElement('div'); unavailable.className = 'v38-oc-unavailable'; unavailable.textContent = 'ローソク足履歴未取得'; host.appendChild(unavailable); return;
    }
    if (!lib || typeof lib.createChart !== 'function') {
      fallbackCandles(host, candles, levels); return;
    }
    const chart = lib.createChart(host, {
      ...
      rightPriceScale: {borderColor: '#dedbd4', scaleMargins: {top: .08, bottom: .08}},
    ...
    const series = chart.addCandlestickSeries({
      upColor: '#2c9a7b', downColor: '#d9534f', borderUpColor: '#2c9a7b', borderDownColor: '#d9534f', wickUpColor: '#2c9a7b', wickDownColor: '#d9534f', priceLineVisible: false,
    });
    series.setData(candles);
    levels.forEach((level) => series.createPriceLine({
      price: level.value, color: level.color, lineWidth: level.key === 'spot' ? 1 : 2,
      lineStyle: lineStyle(lib, level.style), axisLabelVisible: true, title: level.label,
    }));
    chart.timeScale().fitContent();
    currentChart = chart;
    if (window.ResizeObserver) {
      currentObserver = new ResizeObserver(() => { if (currentChart === chart) chart.applyOptions({width: host.clientWidth, height: host.clientHeight}); });
  ...
  function renderLegend(modal, row) {
    ...
    host.replaceChildren();
    const label = document.createElement('span'); label.textContent = 'V38 Options'; host.appendChild(label);
    levelsFromRow(row || {}).forEach((level) => { const item=document.createElement('span');item.className='v38-oc-legend-item';item.style.color=level.color;const sw=document.createElement('i');sw.className='v38-oc-legend-swatch';const text=document.createElement('span');text.textContent=level.label+' '+money(level.value);item.appendChild(sw);item.appendChild(text);host.appendChild(item); });
  }

  function renderStatus(modal, found) {
    const host = modal.querySelector('.v38-oc-status');
    if (!found || !found.row) { host.textContent = '現行チェーン未取得'; return; }
    const row = found.row;
    const parts = [];
    parts.push(found.historical ? '前回実測' : String(found.bucket || '0-45') + ' DTE');
    const em = finite(row.expected_move_pct); if (em !== null) parts.push('Expected Move ' + pct(em));
  ...
  function renderBucketButtons(modal, options, ticker, selected) {
    const host = modal.querySelector('.v38-oc-buckets'); host.replaceChildren(); const available = new Set(availableBuckets(options, ticker));
    BUCKETS.forEach((bucket) => { const button=document.createElement('button');button.type='button';button.className='v38-oc-bucket-btn';button.textContent=bucket;button.disabled=!available.has(bucket);button.setAttribute('aria-pressed',bucket===selected?'true':'false');button.addEventListener('click',()=>{if(button.disabled)return;activeBucket=bucket;const found=resolveRow(options,ticker,bucket);renderBucketButtons(modal,options,ticker,bucket);renderLegend(modal,found&&found.row);renderStatus(modal,found);renderCandles(modal,options,ticker,found&&found.row);});host.appendChild(button); });
  }

  function openTicker(ticker) {
    ticker = cleanTicker(ticker); if (!ticker) return;
    const modal = ensureModal(); lastTicker = ticker; activeBucket = null; activeOptions = null; modal.hidden = false; modal.querySelector('.v38-oc-title').textContent = ticker;
    modal.querySelector('.v38-oc-levels').textContent = 'V38 Options'; modal.querySelector('.v38-oc-status').textContent = 'ローソク足・Options水準を読み込み中…'; modal.querySelector('.v38-oc-chart').replaceChildren();
    loadOptions().then((options) => { if (modal.hidden || lastTicker !== ticker) return; activeOptions=options; const found=resolveRow(options,ticker,null); activeBucket=found&&!found.historical&&BUCKETS.includes(found.bucket)?found.bucket:null; renderBucketButtons(modal,options,ticker,activeBucket); renderLegend(modal,found&&found.row); renderStatus(modal,found); renderCandles(modal,options,ticker,found&&found.row); });
  }

  function closeModal() { const modal=document.getElementById(MODAL_ID); if(modal) modal.hidden=true; disposeChart(); lastTicker=null; activeBucket=null; activeOptions=null; }

  function tickerFromTradingViewHref(link) {
    if (!link || !link.href) return null;
    try { const url=new URL(link.href,window.location.href); if(!/tradingview\.com$/i.test(url.hostname)&&!/\.tradingview\.com$/i.test(url.hostname))return null; return cleanTicker(url.searchParams.get('symbol')); } catch (_) { return null; }
  }

  function tickerFromGenericRow(item) {
    if (!item) return null;
    const name=item.querySelector('.rsx-name'); if(name){const key=String(name.querySelector('small')&&name.querySelector('small').textContent||'').trim().toLowerCase();const value=cleanTicker(name.querySelector('b')&&name.querySelector('b').textContent);if((key==='ticker'||key==='symbol')&&value)return value;const link=name.querySelector('.v38-generic-ticker');const linked=cleanTicker(link&&link.textContent);if(linked)return linked;}
    const spans=Array.from(item.querySelectorAll('.rsx-sub .rsx-nums')); for(const span of spans){const match=String(span.textContent||'').trim().match(/^(?:ticker|symbol)\s+([A-Z][A-Z0-9.\-]{0,9})$/i);if(match)return cleanTicker(match[1]);}
    const text=String(item.textContent||'').toUpperCase();const explicit=text.match(/(?:TICKER|SYMBOL)\s+([A-Z][A-Z0-9.\-]{0,9})/);return explicit?cleanTicker(explicit[1]):null;
  }

  function tickerFromElement(target) {
    if (!(target instanceof Element)) return null;
    const explicitData=target.closest('[data-v38-ticker],[data-v38-rs-ticker],[data-ticker],[data-symbol]'); if(explicitData){const value=explicitData.getAttribute('data-v38-ticker')||explicitData.getAttribute('data-v38-rs-ticker')||explicitData.getAttribute('data-ticker')||explicitData.getAttribute('data-symbol');const ticker=cleanTicker(value);if(ticker)return ticker;}
    const link=target.closest('a'); if(link){if(link.classList.contains('v38-ticker-link')){const ticker=cleanTicker(link.textContent);if(ticker)return ticker;}const ticker=tickerFromTradingViewHref(link);if(ticker)return ticker;}
    const generic=target.closest('.rsx-item');const genericTicker=tickerFromGenericRow(generic);if(genericTicker)return genericTicker;
    const atom=target.closest('.mvr-t,.tk,.chip');if(atom){const ticker=cleanTicker(atom.textContent);if(ticker)return ticker;}
    const item=target.closest('.slrow,.l');if(item){const text=String(item.textContent||'').toUpperCase();const explicit=text.match(/(?:TICKER|SYMBOL)\s+([A-Z][A-Z0-9.\-]{0,9})/);if(explicit)return cleanTicker(explicit[1]);}
    return null;
  }
  ...
  document.addEventListener('click', (event) => { if(!isTickerActionTarget(event.target))return;const ticker=tickerFromElement(event.target);if(!ticker)return;event.preventDefault();event.stopPropagation();openTicker(ticker); }, true);
