...
const title=c=>{if(!c)return'';if(c.dataset.v38CardTitle)return norm(c.dataset.v38CardTitle);const h=c.querySelector('h2,.hdr h2,.chd h2,h4,.th h4');return h?norm(h.textContent):'';};
...
const show=c=>{if(!c)return;c.hidden=false;c.classList.remove('v38-py-unbound');c.removeAttribute('aria-hidden');c.dataset.v38UiSource=SOURCE;};
const hide=c=>{if(!c)return;c.hidden=true;c.classList.add('v38-py-unbound');c.setAttribute('aria-hidden','true');};
...
function rrgBlocks(){
 const out=[];
 TABS.forEach(id=>{const s=document.getElementById(id);if(!s)return;
  Array.from(s.querySelectorAll('.card,.t')).forEach(n=>{const t=title(n);if(/RRG/i.test(t)||t.includes('セクター・ローテーション（テーマETF）')||t.includes('順位フロー（大分類）')||n.querySelector('#bump,#rrg,.rrgtog,.rrg-per,.rrgseed,.rrgq'))out.push(n.closest('.card')||n);});
 });
 return Array.from(new Set(out));
}
function repairRrg(){rrgBlocks().forEach(hide);}
function publishRrgCount(p){if(!p)return 0;return Array.from(p.querySelectorAll('iframe.postframe')).filter(f=>{const src=f.getAttribute('srcdoc')||'';return src.includes('順位フロー（大分類）')&&src.includes('id="bump"');}).length;}
...
function audit(){const r={source:SOURCE,options_touched:false,tabs:{},missing:[],duplicates:[],rrg:[]};Object.entries(EXPECTED).forEach(([id,ns])=>{const cs=cards(id),checks=[];ns.forEach(n=>{const xs=cs.filter(c=>hit(c,n)&&!c.hidden&&!c.classList.contains('v38-py-unbound'));checks.push({needle:n,count:xs.length});if(!xs.length)r.missing.push(`${id}:${n}`);if(xs.length>1)r.duplicates.push(`${id}:${n}:${xs.length}`);});r.tabs[id]={visible:cs.filter(c=>!c.hidden&&!c.classList.contains('v38-py-unbound')).map(title),checks};});const rr=rrgBlocks().filter(c=>!c.hidden&&!c.classList.contains('v38-py-unbound'));r.rrg=rr.map(c=>`${c.closest('section')?.id}:${title(c)||'RRG'}`);if(rr.length)r.duplicates.push(`RRG_TOPLEVEL:${rr.length}`);if(document.querySelectorAll('#t-market #mri-bd').length!==1)r.missing.push('t-market:MC57主要観測表');const p=document.getElementById('t-post1'),pc=p?p.querySelectorAll(':scope > .postwrap').length:0,pubRrg=publishRrgCount(p);r.tabs['t-post1'].postwraps=pc;r.tabs['t-post1'].publish_rrg=pubRrg;r.rrg.push(`t-post1:publish-detail:${pubRrg}`);if(pc!==2)r.missing.push(`t-post1:postwrap:${pc}`);if(pubRrg<1)r.missing.push(`t-post1:順位フロー:${pubRrg}`);if(pubRrg>1)r.duplicates.push(`t-post1:順位フロー:${pubRrg}`);r.ok=!r.missing.length&&!r.duplicates.length;window.V38BuildDashboard3Audit=r;document.documentElement.dataset.v38BuildDashboard3Audit=r.ok?'ready':'error';return r;}
...
function wait(){return new Promise(res=>{const st=performance.now();function q(){const binder=document.documentElement.dataset.v38CanonicalBinder,final=document.body?.dataset.v38PySourceFinal;if((binder==='ready'||binder==='error')&&final==='ready')return res();if(performance.now()-st>15000)return res();requestAnimationFrame(q);}q();});}
