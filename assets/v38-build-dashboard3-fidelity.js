...
function repairRrg(){const s=document.getElementById('t-rotation');if(!s)return;const xs=rrgBlocks().sort((a,b)=>score(b,'t-rotation')-score(a,'t-rotation'));if(!xs.length)return;const keep=xs[0];show(keep);if(keep.closest('section')?.id!=='t-rotation'){const h=s.querySelector(':scope > .msec[data-v38-py-source="1"]');if(h)h.after(keep);else s.prepend(keep);}xs.slice(1).forEach(hide);keep.dataset.v38BuildDashboard3Role='rotation-rrg';}
