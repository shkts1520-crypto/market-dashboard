/* Clone-only TradingView fallback for source-mc57.html. Do not load on index.html. */
(function () {
  'use strict';
  ...
  function apply() {
    ...
    load().then(({options, search}) => {
      ...
      if (empty) {
        ...
        if (status) status.textContent = 'TradingView 実チャート • EMA21 • ローカル優先履歴外はライブ表示へ自動切替';
      }
