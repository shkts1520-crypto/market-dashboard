...
def test_f1_20_observable_5_drop_is_25pct_and_83pct_partial():
    ...
    rows = []
    # Only T00..T19 are observable from the old cohort. T00..T04 lose base-pool eligibility.
    for i in range(20):
        rows.append(row(f"T{i:02d}", rs189=100 - i * 0.1, sma50=(90.0 if i < 5 else 110.0)))
    # Extra current names ensure a meaningful current ranking but do not alter the five eligibility drops.
...
def test_f1_refuses_unproven_old_top24_provenance():
    bad = {"session_date": "2026-08-11", "rows": [{"ticker": "AAA"}]}
...
def test_f2_valid20_below85_8_is_40pct_severe():
    rows = []
    for i in range(30):
        # Current base-pool rank is deterministic by rs189 desc.
