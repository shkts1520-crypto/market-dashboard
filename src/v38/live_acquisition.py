...
CALCULATION_VERSION = "v38-live-acquisition-1.0.0"
...
def choose_completed_session(qqq_dates: Iterable[str], spy_dates: Iterable[str], *, now_utc: datetime | None = None) -> str:
    ...
    latest = candidates[-1]
    if latest == ny.date().isoformat() and ny.time() < dtime(16, 10):
        if len(candidates) < 2:
...
def adjusted_ohlcv_rows(frame: pd.DataFrame, *, ticker: str, target_session: str) -> list[dict[str, Any]]:
    ...
    for idx, raw in f.iterrows():
        ...
        if day > target_session:
            continue
        close, adj = _finite(raw.get("close")), _finite(raw.get("adj_close")) if "adj_close" in f.columns else None
        ...
        for offset in range(0, len(tickers), chunk_size):
            ...
            for round_no in range(2):
                ...
                for start in range(0, len(missing), 20):
                    sub = missing[start:start + 20]; sub_map = {t: yahoo_symbol(t) for t in sub}
                    try:
            ...
            for ticker in originals:
                ...
                if rows:
                    history_ok += 1; writer.writerows(rows)
                if any(r["date"] == target_session and r["close"] is not None for r in rows):
                    target_ok += 1
                else:
...
def write_universe_csv(path: str | Path, rows: list[dict[str, Any]]) -> Path:
    ...
    with p.open("w", newline="", encoding="utf-8") as handle:
        w = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore"); w.writeheader(); w.writerows(rows)
    return p
...
def state_object(*, session_date: str, generated_at: str, coverage: float) -> dict[str, Any]:
    return {"session_date": session_date, "generated_at": generated_at, "coverage": coverage,
            "source": "authoritative-session:QQQ+SPY; universe:TradingView; prices:Yahoo",
            "schema_version": STATE_SCHEMA_VERSION, "calculation_version": CALCULATION_VERSION, "status": "READY"}
