...
MAX_VALID_IV = 5.0
MIN_READY_TARGET_COVERAGE = 0.25
MIN_READY_TICKERS = 3
...
def bs_gamma(
    ...
    if any((not math.isfinite(float(x)) for x in values)):
        return None
    if spot <= 0 or strike <= 0 or time_years <= 0 or implied_volatility <= 0 or implied_volatility > MAX_VALID_IV:
        return None
...
def frame_to_contracts(
    *,
    ...
    for _, raw in frame.iterrows():
        ...
        if (
            strike is None or strike <= 0
            or oi is None or oi <= 0
            or iv is None or iv <= 0 or iv > MAX_VALID_IV
        ):
            continue
        out.append(
            Contract(
...
def aggregate_bucket(
    *,
    ...
    quality = "GOOD" if len(contracts) >= 30 and quote_coverage >= 0.30 else "PARTIAL"
    return {
        "ticker": ticker,
        "bucket": bucket,
        "spot": float(spot),
...
def build_options_index(
    *,
    ...
    successes = len({str(row["ticker"]) for row in bucket_rows["0-45"]})
    ...
    status = (
        "READY"
        if successes >= MIN_READY_TICKERS and coverage >= MIN_READY_TARGET_COVERAGE
        else "DATA_REQUIRED"
    )
    return {
        ...
        "status": status,
        "reason": None if status == "READY" else "OPTION_CHAIN_COVERAGE_BELOW_MINIMUM",
        "target_policy": {
        ...
        "readiness_contract": {
            "min_success_tickers": MIN_READY_TICKERS,
            "min_target_coverage": MIN_READY_TARGET_COVERAGE,
            "invalid_iv_above": MAX_VALID_IV,
        },
        "risk_free_rate": rate,
